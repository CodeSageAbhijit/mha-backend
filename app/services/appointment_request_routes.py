...
router = APIRouter(prefix="/api", tags=["Appointment Requests"])  # separate tag
...
async def get_psychiatrist_availability(
    ...
    try:
        ...
        try:
            ...
            return error_response(422, "Invalid date format", "ValidationError", ["Expected date YYYY-MM-DD"])
...
async def request_appointment(
    ...
    try:
        ...
        # Create a request without date/time; doctor will decide later
        doc = {
            "appointmentId": f"APPT-{int(datetime.utcnow().timestamp())}",
            "patientId": body.patientId,
            "doctorId": body.doctorId,
            "timeSlot": None,  # to be set by doctor when accepting
            "date": None,      # to be set by doctor when accepting
