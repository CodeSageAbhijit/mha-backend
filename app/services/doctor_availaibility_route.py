...
async def create_availability(
    ...
    if payload.doctor_id != current_user["userId"]:
        raise HTTPException(status_code=403, detail="You can only create your own availability")

    try:
        # Check if availability already exists
        existing = await psychiatrist_availability_collection.find_one({"doctor_id": payload.doctor_id})
        if existing:
...
async def get_availability(doctor_id: str):
    print(doctor_id)
    doc = await psychiatrist_availability_collection.find_one({"doctor_id": doctor_id})
    print(doc)
    if not doc:
...
async def delete_availability(
    ...
    result = await psychiatrist_availability_collection.delete_one({"doctor_id": doctor_id})
