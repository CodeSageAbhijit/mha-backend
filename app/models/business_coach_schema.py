...
class BusinessCoachBase(BaseModel):
    firstName: str
    lastName: str
    email: EmailStr
    dateOfBirth: str
    ...
    specialization: Optional[List[str]] = Field(default_factory=list, description="Business expertise areas")
    problemTypes: Optional[List[str]] = Field(default_factory=list, description="Business problems (e.g., startup, scaling, marketing, sales, finance)")
    experienceYears: Optional[int] = 0
    profilePhoto: str
    consultationFee: Optional[int] = 1000
...
class BusinessCoachCreate(BusinessCoachBase):
    pass

class BusinessCoachUpdate(BaseModel):
...
class BusinessCoachResponse(BusinessCoachBase):
